    ...
    public partial class Entradas : System.Web.UI.Page
    {
        ...
        void Permisos(int id_rol)
        {
            try
            {
                ...
                while (reader.Read())
                {
                    foreach (GridViewRow fila in gvEntrada.Rows)
                    //while (reader.Read())
                    {
        ...
        void Datos()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("sp_SelEntrada", con);
                cmd.CommandType = CommandType.StoredProcedure;
                con.Open();
        ...
        {
            Response.Redirect("~/Pages/AdministracionProductos/Entradas/EDetalle.aspx?op=C");
        }
        ...
        protected void Btnread_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Pages/AdministracionProductos/Entradas/EDetalle.aspx?id=" + id + "&op=R");
        }

        protected void BtnTodos_Click(object sender, ImageClickEventArgs e)
        {
            Response.Redirect("~/Pages/AdministracionProductos/Entradas/TEntradas.aspx");
        }
    }
}